...
class ardupilot_rbx_fake_gps_process(object):
  ...
  def __init__(self):
    ...
    # NEPI RBX Namespace
    # Wait for rbx status topic to publish
    rbx_status_topic = "/rbx/status"
    rospy.loginfo("Waiting for topic name: " + rbx_status_topic)
    rbx_topic=nepi.wait_for_topic(rbx_status_topic)
    RBX_NAMESPACE = (rbx_topic.rpartition("rbx")[0] + "rbx/")
    rospy.loginfo("Found rbx namespace: " + RBX_NAMESPACE)
  ...
  def reset_gps(self):
    ...
    self.fake_gps_enabled = False
    nepi.sleep(MOVE_UPDATE_TIME_SEC,100)
    self.fake_gps_enabled = True
  ...
  def rbx_goto_position_callback(self,position_cmd_msg):
    ...
    new_geopoint_wgs84=get_geopoint_at_body_point(self.current_location_wgs84_geo, \
  ...
  def takeoff(self):
    new_alt_m = self.current_location_wgs84_geo.altitude + TAKEOFF_ALT_M
    new_geopoint_wgs84=GeoPoint()
  ...
  def rtl(self):
    global current_home_wgs84_geo
    mavlink_fake_gps_move(current_home_wgs84_geo)
