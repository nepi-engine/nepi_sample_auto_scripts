...
# 1. starts a list of scripts using NEPI ROS service
# 2. waits set time
# 3. stops a list of scripts using NEPI ROS service
...
### Function to stop scripts from list
def stop_scripts(script_list):
  installed_scripts = get_installed_scripts()
  running_scripts = get_running_scripts()
  if installed_scripts is not None and running_scripts is not None:
    for script2stop in script_list:
      script_installed = val_in_list(script2stop,installed_scripts)
      if script_installed:
...
def startNode():
  ...
  #launch_script(SCRIPT_LIST[0])
  # Wait for 5 seconds to check in RUI
  time.sleep(5)
  # Stop scripts from list
  stop_scripts(SCRIPT_LIST)
  # Wait for 5 seconds to check in RUI
  time.sleep(5)
  # Relaunch scripts that were running at start that were stopped
  print("Relaunching scripts that were running at start")
  launch_scripts(scripts_running_at_start)  
